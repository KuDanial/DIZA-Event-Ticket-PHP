...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (mysqli_fetch_assoc(mysqli_stmt_get_result($stmt_chk))) {
        ...
        try {
            $insert = "INSERT INTO users (username, password_hash, first_name, last_name, email, phone_number, gender, address, role, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'attendee', 'active')";
            ...
            $stmt_det = mysqli_prepare($conn, "INSERT INTO attendee_details (user_id, student_id) VALUES (?, ?)");
            mysqli_stmt_bind_param($stmt_det, "is", $user_id, $student_id);
            mysqli_stmt_execute($stmt_det);

            mysqli_commit($conn);
            header("Location: manage-attendees.php?success=Attendee+created+successfully");
            exit();
        } catch (Exception $e) {
            mysqli_rollback($conn);
            $error = 'Failed to create attendee account.';
        }
    }
}
?>
    ...
    <div class="dashboard-container">
        ...
        <main class="main-content">
            <header class="top-bar">
                <div class="page-title"><h1>Add New Attendee</h1></div>
                <div class="user-menu">
            ...
            <div class="content-padding">
                <div class="card card-padded">
                    ...
                    <form action="manage-attendees-addNew.php" method="POST" class="admin-form">
                        <?php if ($error): ?>
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group">
                                <label>Student ID / Card Number <span>*</span></label>
                                <input type="text" name="studentId" class="form-input" placeholder="e.g. 2023123456" required>
                            </div>
                        </div>
                        ...
                        <div class="form-actions">
                            ...
                            <button type="submit" class="btn-primary"><i class="fas fa-save"></i> Create Attendee</button>
                        </div>
                    </form>
                </div>
            </div>
